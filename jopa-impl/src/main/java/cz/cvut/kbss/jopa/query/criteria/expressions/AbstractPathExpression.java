...
abstract public class AbstractPathExpression<X> extends AbstractExpression<X> implements Path<X> {
    ...
    public AbstractPathExpression(Class<X> type, AbstractPathExpression pathSource, Metamodel metamodel, CriteriaFactory factory) {
        super(type, factory);
    ...
    public <Y> Path<Y> getAttr(String attributeName) throws IllegalArgumentException {
        ...
        Path<Y> newPathSource = new PathImpl<Y>(this.metamodel,new ExpressionAttributeImpl(attribute.getJavaType(),  this.pathSource, this.metamodel, attribute,this.factory),attribute.getJavaType(),this.factory);
