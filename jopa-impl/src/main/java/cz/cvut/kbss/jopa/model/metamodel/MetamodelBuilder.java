...
public class MetamodelBuilder {
    ...
    private <X> void processManagedType(TypeBuilderContext<X> context) {
        ...
        final FieldAndMethodSparqlQueryProcessor<X> fieldAndMethodQueryProcessor = new FieldAndMethodSparqlQueryProcessor<>(context, this); //TODO make class attr

        for (Field f : cls.getDeclaredFields()) {
            fieldProcessor.processField(f);

            fieldAndMethodQueryProcessor.processField(f);
        }

        for (Method m : cls.getDeclaredMethods()) {
            fieldAndMethodQueryProcessor.processMethod(m);
        }
