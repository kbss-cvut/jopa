...
public class SesameMemoryStoreMultiContextQueryTest {
	...
	// Logger.getLogger(SesameMemoryStoreMultiContextQueryTest.class
	// .getName());
	//
	// private static URI CONTEXT_ONE =
	// URI.create("http://krizik.felk.cvut.cz/ontologies/contextOne");
	// private static URI CONTEXT_TWO =
	// URI.create("http://krizik.felk.cvut.cz/ontologies/contextTwo");
	//
	...
	// em =
	// TestEnvironment.getPersistenceConnector("SesameMultiContextSPARQLQueryTests",
	// storage,
	// false, properties);
	...
	// final OWLClassA a = QueryTestEnvironment.getDataByContext(CONTEXT_ONE,
	// OWLClassA.class)
	// .get(0);
	// final OWLClassA aa = em
	// .find(OWLClassA.class, a.getUri(), new EntityDescriptor(CONTEXT_ONE));
	// assertNotNull(aa);
	// final OWLClassA ac = QueryTestEnvironment.getDataByContext(CONTEXT_TWO,
	// OWLClassA.class)
	// .get(0);
	// final OWLClassA aac = em.find(OWLClassA.class, ac.getUri(), new
	// EntityDescriptor(
	// CONTEXT_ONE));
	// assertNull(aac);
