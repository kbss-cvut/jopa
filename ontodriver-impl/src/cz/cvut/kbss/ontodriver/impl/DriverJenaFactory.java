...
public class DriverJenaFactory extends DriverAbstractFactory {
	...
	static {
		try {
			OntoDriverImpl.registerFactoryClass(OntologyConnectorType.JENA,
					DriverJenaFactory.class);
	...
	public StorageModule createStorageModule(Context ctx, boolean autoCommit)
		...
		final JenaStorageModule m = new JenaStorageModule(ctx);
		registerModule(m);
		return m;
	}

	@Override
	public JenaStorageConnector createStorageConnector(Context ctx,
			boolean autoCommit) throws OntoDriverException {
		ensureState(ctx);
		if (LOG.isLoggable(Level.FINER)) {
			LOG.finer("Creating Jena storage connector.");
		}
		final JenaStorageConnector c = new JenaStorageConnector(
