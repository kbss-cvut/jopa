...
public class ReferencedListHandlerTestWithStorage {
	...
	private Collection<Axiom<?>> generateAxiomsForList(ReferencedListValueDescriptor listDescriptor) {
		...
		for (NamedResource val : listDescriptor.getValues()) {
			...
			Axiom<?> ax = new AxiomImpl<>(node, listDescriptor.getNodeContent(), new Value<URI>(
					val.getIdentifier()));
			axioms.add(ax);
		}
		return axioms;
