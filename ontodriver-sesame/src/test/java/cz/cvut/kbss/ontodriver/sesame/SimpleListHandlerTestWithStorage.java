...
public class SimpleListHandlerTestWithStorage {
	...
	public void persistsSimpleList() throws Exception {
		...
		final Collection<Axiom<?>> axioms = generateAxiomsForList(descriptor);
	...
	private Collection<Axiom<?>> generateAxiomsForList(SimpleListValueDescriptor listDescriptor) {
		final Collection<Axiom<?>> axioms = new ArrayList<>(listDescriptor.getValues().size());
		...
		for (NamedResource val : listDescriptor.getValues()) {
			Axiom<?> ax;
			if (val == previous) {
				ax = new AxiomImpl<>(OWNER, listDescriptor.getListProperty(), new Value<URI>(
						val.getIdentifier()));
			} else {
				ax = new AxiomImpl<>(previous, listDescriptor.getNextNode(), new Value<URI>(
						val.getIdentifier()));
			}
			axioms.add(ax);
			previous = val;
	...
	private void verifyListContent(Collection<Axiom<?>> expected,
			SimpleListDescriptor listDescriptor) throws Exception {
		final Collection<Axiom<?>> actual = handler.loadList(listDescriptor);
		assertEquals(expected.size(), actual.size());
		// This is more explicit on failure than just containsAll
		final Iterator<Axiom<?>> itExp = expected.iterator();
		final Iterator<Axiom<?>> itAct = actual.iterator();
		while (itExp.hasNext()) {
