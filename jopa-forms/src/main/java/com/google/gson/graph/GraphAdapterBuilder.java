...
public final class GraphAdapterBuilder {
  ...
  public Factory registerOn(GsonBuilder gsonBuilder) {
  ...
  static public class Factory implements TypeAdapterFactory, InstanceCreator {
    public final Map<Type, InstanceCreator<?>> instanceCreators;
    public final ThreadLocal<Graph> graphThreadLocal = new ThreadLocal<Graph>();
    ...
    public <T> TypeAdapter<T> create(final Gson gson, TypeToken<T> type) {
      ...
      return new TypeAdapter<T>() {
        @Override public void write(JsonWriter out, T value) throws IOException {
          ...
          if (element == null) {
            element = new Element<T>(value, graph.nextName(value), typeAdapter, null);
        ...
        @Override public T read(JsonReader in) throws IOException {
          ...
          try {
            ...
            if (readEntireGraph) {
                for (Element<?> el : graph.map.values()) {
                    if (el.value == null) {
//                        System.out.printf("null %s - %s\n", el.id, el.typeAdapter.toString());
//                        if (el.typeAdapter == null) {
//                            System.out.println(el.element.getAsJsonObject().get("$class"));
//                        }
                        Class cls = null;
                        try {
                            cls = Class.forName(el.element.getAsJsonObject().get("$class").getAsString());
                        } catch (ClassNotFoundException ex) {
                            Logger.getLogger(GraphAdapterBuilder.class.getName()).log(Level.SEVERE, null, ex);
                        }
                        el.typeAdapter = gson.getDelegateAdapter(factory, TypeToken.get(cls));
                        el.read(graph);
                    }
                }
            //  graphThreadLocal.remove();
  ...
  static public class Graph {
     ...
     */
    public String nextName(Object value) {
       //Gson gson = new Gson();
